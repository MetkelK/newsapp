...
class App extends Component {
	...
	getArticles(){
  	...
  	searchArticles = (e) => {
    	e.preventDefault();
    	fetch(`${NYT_SEARCH_URL}${this.state.query}&api-key=${process.env.REACT_APP_API_KEY}`)
      		.then((res) => res.json())
      		.then((res) => {
  	...
  	render() {
    	return (
      		<React.Fragment>
        		<Navbar handleSearch={this.handleSearch} searchArticles={this.searchArticles}></Navbar>
        		...
        		<p style={{ textAlign: 'center', margin: '1rem 0' }}>Data provided by <a href="https://www.nytimes.com" target="_blank" rel="noopener noreferrer">The New York Times</a></p>
      		</React.Fragment>
    	);
  	}
}
